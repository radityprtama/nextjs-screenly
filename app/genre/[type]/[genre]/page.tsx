...
export default async function GenrePage({
  ...
  const res = await idlixApi.getGenre(type, genre, currentPage);
  const rawItems = res?.data || [];
  const items = rawItems.map((i: any) => normalizeIdlixItem(i, type));
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
      <div>
        <h1 className="text-3xl font-extrabold tracking-tight capitalize">
          {genre.replace("-", " ")} {type === "series" ? "TV Series" : "Movies"}
        </h1>
        <p className="text-white/50 text-sm mt-1">Catalog listings filtered by genre.</p>
      </div>
      ...
      <div className="flex justify-center items-center gap-4 pt-6 border-t border-white/5">
        <Link
          href={`/genre/${type}/${genre}?page=${Math.max(1, currentPage - 1)}`}
        ...
        <Link
          href={`/genre/${type}/${genre}?page=${currentPage + 1}`}
          className={`px-4 py-2 rounded-full text-xs font-bold border transition-colors ${
            !hasNext
              ? "bg-white/5 border-white/10 text-white/30 pointer-events-none"
              : "bg-white/5 border-white/10 hover:bg-white/10 text-white"
